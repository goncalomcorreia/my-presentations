...
\begin{frame}
    ...
    \uncover<5->{\cornercite{brown1993mathematics}}
    \begin{columns}
        \uncover<1->{
            \hspace{2mm}\vspace{-1cm}
            
        \begin{column}{0.5\columnwidth}
        \vspace{0.5cm}
        }
        \begin{itemize}
            \uncover<1->{\item Transparency}
            \uncover<2->{\item Sparsity}
            \uncover<3->{\item Compactness}
            \uncover<4->{\item Data efficiency}
        \end{itemize}
        \end{column}
    
        \begin{column}{0.5\columnwidth}
        \begin{center}
            \uncover<5->{\includegraphics[width=0.9\columnwidth]{figures/alignments.pdf}}
        \end{center}
        \end{column}
        \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \uncover<1->{\item Advances in optimization $\rightarrow$ ditch structure in favour of direct decomposition}
        \uncover<2->{\item Good for high-resource language, fails in low-resource settings}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \bigskip
        \begin{itemize}
            \uncover<2->{\item Inspired by, and data efficient like APE}
            \uncover<3->{\item Transparent like sparse transformers}
            \uncover<4->{\item Compact thanks to latent variables}
            \uncover<5->{\item Using sparsity as a training method}
        \end{itemize}
        \end{column}
