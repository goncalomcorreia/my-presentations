...
\begin{frame}
    ...
    \begin{itemize}
        \item[] What about in MT?
    \end{itemize}
    \vspace{1cm}
    \centering
    \includegraphics[width=0.5\columnwidth]{figures/alignment_struct.png}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \uncover<4->{\item[] but mostly only continuous latent variables}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \fontsize{13pt}{15}\selectfont%
    \begin{itemize}
        \uncover<1->{\item[] Attention can be seen as learning a soft structure}
        \uncover<2->{\item attention between the src and tgt is similar to the alignments we've seen before}
    \end{itemize}

    \vspace{1cm}
    \centering
    \uncover<2->{\includegraphics[width=0.5\columnwidth]{figures/alignment_struct.png}}

    \vspace{1cm}
    \begin{itemize}
        \uncover<3->{\item[] Spoiler: we will see that many attention heads have very specific and simple roles...}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \fontsize{13pt}{15}\selectfont%
    \begin{itemize}
        \uncover<1->{\item[] With a memory of phrase alignments between source and target, we can do APE {\it inside} the NMT system.}
    \end{itemize}
...
\begin{frame}
    ...
    \fontsize{13pt}{15}\selectfont%
    \begin{itemize}
        \uncover<1->{\item state of the art for APE via BERT}
        \uncover<2->{\item find that many heads in the Transformer have simple {\it structure} that could be hard-coded}
        \uncover<3->{\item introduce an efficient method to train discrete and structured latent variables}
        \uncover<4->{\item bring latent structure into NMT via draft editing and latent summaries}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Automatic Post-Editing
        \item models with discrete or structured latent variables
        \item low-resource NMT
        \item document-level NMT
    \end{itemize}
\end{frame}
