...
\begin{frame}
    \vspace*{1mm}
    \frametitle{Published work of this thesis}
    \only<2>{\cornerciteme{myape2019}}
    \only<3>{\cornerciteme{mytransf2019}}
    \only<4>{\cornerciteme{myneurips2020}}
    \vspace*{-2.5em}
    \begin{itemize}
        \uncover<2->{\item Automatic Post-Editing using {\color{tPeony} weak supervision} for {\color{myDarkYellow} data-efficiency} (ACL)}
              \uncover<3->{\item Letting transformer {\color{tPeony} learn sparsity} of its attentions for {\color{myDarkYellow} transparency} (EMNLP)}
              \uncover<4->{\item General strategy for efficiently training discrete latent variable models, to have {\color{myDarkYellow} compactness} (NeurIPS)}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \vspace*{5mm}
    \begin{itemize}
        \uncover<2->{\item[] {\color{myDarkYellow} Challenge:} APE data is very scarce! Need to create artificial data.}
    \end{itemize}
\end{frame}
        ...
        \begin{center}
            ...
            \resizebox{0.8\columnwidth}{!}{\begin{tabular}{lrr}
                    ...
                    \uncover<3->{dual-source transformer (23K)
                     & 27.73                        & 59.78                        } \\
                    \uncover<4->{ours (23K)
                     & {\color{myDarkYellow} 19.03} & {\color{myDarkYellow} 70.66} } \\
                    \uncover<5->{ours (8M)
                     & {\color{myDarkYellow} 17.26} & {\color{myDarkYellow} 73.42} } \\
                    \bottomrule
                \end{tabular}}
        \end{center}
    \end{table}
...
\begin{frame}
    ...
    \only<4>{\cornercite{lee2020POSTECHETRISubmissionWMT2020}}
    \only<5>{\cornercite{kodama2020GeneratingResponsesthat}}
    \centering
    \fontsize{12pt}{15}\selectfont
    \begin{itemize}
        \uncover<2->{\item One of pioneers in using pre-trained transformer encoders for a generation task}
              ...
              \uncover<5->{\item Inspired other works that use scarse data (\textit{e.g.}, dialogue with metadata)}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
    \frametitle{Conclusions and impact}
    \uncover<5->{\cornercite{treviso2021PredictingAttentionSparsity, zhang2021SparseAttentionLinear, raganato2020FixedEncoderSelfAttention}}
    ...
    \only<5->{
        \begin{itemize}
            \uncover<5->{
            \item[] Subsequent work has:
            \item focused on taking computational advantage of sparsity
            \item proposed other sparse activations (\textit{e.g.}, ReLU)
            \item incorporated fixed attention patterns that we found.
                  }
        \end{itemize}
    }
    %
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\columnwidth}
            ...
            \begin{itemize}
                \uncover<2->{\item SFE $\rightarrow$ unbiased but high variance}
                      \uncover<4->{\item Gumbel-Softmax $\rightarrow$ continuous relaxation, biased estimation}
            \end{itemize}

            \bigskip

            \begin{itemize}
                \uncover<5->{\item[] New option: {\color{tPeony} use sparsity}!\quad\emoji{palms}}
            \end{itemize}

            \begin{itemize}
                \uncover<7->{\item[] no need for sampling $\rightarrow$ no variance}
                    \uncover<8->{\item[] no continuous relaxation}
            \end{itemize}

        \end{column}
        \vspace{-5cm}\begin{column}{0.45\columnwidth}
            \small%
            \only<1-5>{
                \begin{tabular}{lr@{~}lr}
                    \toprule
                    ...
                    \midrule
                    {\emph{Monte Carlo}}                    &                                  &                                &       \\
                    \uncover<2->{SFE                        & $33.05$                          & {\tiny\color{gray}$\pm 2.84$}  & $1$}  \\
                    \uncover<3->{SFE$+$                     & $44.32$                          & {\tiny\color{gray}$\pm 2.72$}  & $2$}  \\
                    \uncover<3->{NVIL                       & $37.04$                          & {\tiny\color{gray}$\pm 1.61$}  & $1$}  \\
                    \uncover<4->{Gumbel                     & $23.51$                          & {\tiny\color{gray}$\pm 16.19$} & $1$}  \\
                    \uncover<4->{ST Gumbel                  & $27.42$                          & {\tiny\color{gray}$\pm 13.36$} & $1$}  \\
                    \midrule
                    \emph{Marginalization}                  &                                  &                                &       \\
                    Dense                                   & $93.37$                          & {\tiny\color{gray}$\pm 0.42$}  & $256$ \\
                    \uncover<5->{\textcolor{tPeony}{Sparse} &
...
\begin{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{.33\textwidth}
            \centering
            \uncover<2->{
                \textbf{\emph{data-efficiency}}\\[.5\baselineskip]
                \vspace{1cm}
                \begin{center}
                    \resizebox{0.8\columnwidth}{!}{\begin{tabular}{lr}
                            ...
                            model (data size)
                             & BLEU$\uparrow$               \\
                            \midrule
                            dual-source transformer {\color{tPurple}(8M)}
                             & 71.72                        \\
                            dual-source transformer {\color{tBlue}(23K)}
                             & 59.78                        \\
                            ours {\color{tBlue}(23K)}
                             & {\color{myDarkYellow} 70.66} \\
                            \bottomrule
                        \end{tabular}}
                \end{center}}%
        \end{column}
        \begin{column}{.33\textwidth}
            \centering
            \uncover<3->{
                \textbf{\emph{transparency}}\\[\baselineskip]
                \vspace{0.2cm}
                \includegraphics[trim=157mm 17mm 0 0, clip, width=.7\textwidth]{figures/comparison_mybg}}%
        \end{column}
        \begin{column}{.33\textwidth}
            \centering
            \uncover<4->{
                \textbf{\emph{better compactness}}\\[\baselineskip]
                \begin{figure}[ht]
                    \begin{tikzpicture}
                        % DISCRETE
                        \draw[draw=tPink,fill=tPink] (1.4,2) circle (0.2) node[anchor=south, yshift=2mm] {{\color{tPeony} \small 0.2}};
